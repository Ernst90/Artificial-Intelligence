...
# We define the mean (average reward)
...
def add_plot(x, y, name, title, subtitle, labels):
    """ 
    Here it will be defined how we want our plot to be added. Where x will be episodes and y mean reward. 
    For our name we add the corresponding graph along with its problem ID which will be updated. 
    """   
    plt.plot(x, y)
    plt.xlabel(labels[0])
    plt.ylabel(labels[1])
...
def position_to_coordinates(pos, ncol):
    """ 
    Transaltes lochlomond positions into a (horizontal, vertical) system or alternatively, into (x, y) grid coordinates. 
    """       
...
def get_terminals(env):
    """ 
    Gives coordinates of terminal states, that is either holes H or the goal state G within our 8x8 grid world. 
    """    
...
def to_grid(mapping, rows, cols):
    """ Based on GridMDP(MDP) from AIMA Toolbox """
